...
namespace Vistian.Reactive.Proxy.Utils
{
    public static class ConnectionFactory
    {
        ...
        static Func<object, OperatorInfo, object> CreateConnectionFactory(Type pt)
        {
            ...
            {
                var signalType = pt.GetTypeInfo().GenericTypeArguments[0];
            ...
            else if (pt.IsArray)
            {
                ...
                var signalType = observableType.GetTypeInfo().GenericTypeArguments[0];
                ...
                IsGenericTypeDefinition(pt.GetTypeInfo().GenericTypeArguments[0], typeof(IObservable<>)))
            {
                var observableType = pt.GetTypeInfo().GenericTypeArguments[0];
                var signalType = observableType.GetTypeInfo().GenericTypeArguments[0];
        ...
        static object TryCreateObservableConnection(object source, Type signalType, OperatorInfo operatorInfo)
        {
            ...
            var ctor = _connectionConstructorCache.GetOrAdd(signalType, _ => new Lazy<ConstructorInfo>(() => GetConnectionConstructor(signalType)));

            return ctor.Value.Invoke(new object[] { RxTraceContext.Current, source, operatorInfo });
        }
        ...
        static ConstructorInfo GetConnectionConstructor(Type signalType)
        {
            ...
            //return operatorObservable.GetTypeInfo()GetConstructor(new[] { typeof(IRxSpySession), typeof(IObservable<>).MakeGenericType(signalType), typeof(OperatorInfo) });

            return operatorObservable.GetTypeInfo().DeclaredConstructors.ToArray()[0];
        }

        static bool IsGenericTypeDefinition(Type source, Type genericTypeComparand)
        {
            return source.GetTypeInfo().IsGenericType && source.GetGenericTypeDefinition() == genericTypeComparand;
        }
