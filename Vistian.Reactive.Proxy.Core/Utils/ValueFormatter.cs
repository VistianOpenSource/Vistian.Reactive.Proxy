...
namespace Vistian.Reactive.Proxy.Utils
{
    public static class ValueFormatter
    {
        ...
        private static Func<object, string> BuildFormatterDelegate(Type type)
        {
            ...
            if (typeof(System.Collections.IList).GetTypeInfo().IsAssignableFrom(type.GetTypeInfo()))
            {
                string typeName = TypeUtils.ToFriendlyName(type);
                return o =>
                {
                    ...
                    {
                        return typeName + " {" + string.Join((string) ", ", (IEnumerable<string>) list.Cast<object>().Select(ToString)) + "}";
                    }
                    else
                    {
