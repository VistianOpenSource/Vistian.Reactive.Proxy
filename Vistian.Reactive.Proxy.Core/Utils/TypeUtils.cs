        ...
        static string toFriendlyNameImpl(Type type)
        {
            ...
            if (ti.IsGenericType)
            {
                if (ti.GetGenericTypeDefinition() == typeof(Nullable<>))
                {
                    ...
                    return ToFriendlyName(ti.GenericTypeParameters[0]) + "?";
                }
                else
                {
                    var definition = type.GetGenericTypeDefinition();

                    return GetNameWithoutGenerics(definition) + "<" + string.Join((string) ", ", (IEnumerable<string>) ti.GenericTypeParameters.Select(ToFriendlyName)) + ">";
                }
            }
