...
namespace Vistian.Reactive.Proxy
{
    ...
    internal class QueryLanguageProxy : RealProxy, IRemotingTypeInfo
    {
        ...
        internal QueryLanguageProxy(Session session, object realQueryLanguage)
        ...
        public override IMessage Invoke(IMessage msg)
        {
            ...
            if (!Session.Current.IsCapturing)
            ...
            if (!_skipFrameCount.TryGetValue(method, out skipFrames))
            {
                ...
                {
                    var dt = frames[i].GetMethod();

                    if (IsRemotingWrapper(dt))
                    {
                        for (; i < frames.Length; i++)
                        {
                            if (!IsRemotingWrapper(frames[i].GetMethod()))
                            {
            ...
            var callSite = skipFrames == -1 ? null : CallSiteCache.Get(skipFrames+1);
            var handler = GetHandler(call, method, callSite);

            return handler(call);
        }

        private bool IsRemotingWrapper(System.Reflection.MethodBase method)
        {
            return method.DeclaringType == typeof(System.Object) && method.Name == "__icall_wrapper_mono_remoting_wrapper";
        }
